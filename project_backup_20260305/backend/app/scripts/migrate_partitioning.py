    ...
    async with SessionLocal() as db:
        for table_name, config in tables.items():
            try:
                ...
                res = await db.execute(text(f"SELECT relkind FROM pg_class WHERE relname = '{table_name}'"))
                row = res.fetchone()
                if row and row[0] == 'p':
                ...
                # 1. Rename old table
                await db.execute(text(f"ALTER TABLE {table_name} RENAME TO {table_name}_old"))
                ...
                await db.execute(text(f"CREATE TABLE {table_name}_default PARTITION OF {table_name} DEFAULT"))
                
                # 4. Migrate data
                await db.execute(text(f"INSERT INTO {table_name} ({config['columns']}) SELECT {config['columns']} FROM {table_name}_old"))
                ...
                await db.execute(text(f"SELECT setval('{seq_name}', (SELECT max({config['pk_col']}) FROM {table_name}))"))
                await db.execute(text(f"ALTER TABLE {table_name} ALTER COLUMN {config['pk_col']} SET DEFAULT nextval('{seq_name}')"))
