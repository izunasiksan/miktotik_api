...
const ResourceUsageCard = ({ id, data, isCompact, cardPadding, scopeLabel, granularityLabel, isLoading }) => {
  const chartId = id || 'resource-usage-chart';
  ...
  return (
    <div id={chartId} className={`bg-white ${cardPadding} rounded-2xl shadow-sm border border-gray-100 h-full relative group`}>
      <div className="flex justify-between items-center mb-4">
        <div>
          <h3 className="text-lg font-bold text-gray-700">Resource Usage (%)</h3>
          {isLoading ? (
        ...
        {!isLoading && (
          <button 
            onClick={() => downloadElementAsImage(chartId, `resource-usage-${new Date().getTime()}.png`)}
            className="p-1.5 text-gray-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all opacity-0 group-hover:opacity-100"
