...
const HabitPatternAnalysis = () => {
  ...
  const { hod_profile, dow_profile, stability_metrics, metadata = {} } = analysisData.stage4;

  // Format labels for HOD (00:00 - 23:00)
  const formattedHodData = hod_profile?.map(item => ({
    ...item,
  ...
  const formattedDowData = dow_profile?.map(item => ({
    ...item,
  ...
  const stabilityScore = stability_metrics?.stability_score ?? 0;
  const isStable = stabilityScore >= 70;
  const sampleDays = metadata.sample_days ?? 0;
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
              Peak: {stability_metrics?.peak_hour?.toString().padStart(2, '0')}:00
            </span>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={formattedHodData}>
                ...
                <Area 
                  ...
                  fill="url(#colorValue)" 
                />
                {stability_metrics?.peak_hour !== undefined && (
                  <ReferenceLine x={formattedHodData[stability_metrics.peak_hour]?.displayTime} stroke="#f43f5e" strokeDasharray="3 3" label={{ position: 'top', value: 'PEAK', fill: '#f43f5e', fontSize: 8, fontWeight: 'bold' }} />
                )}
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 hover:bg-white transition-all cursor-default group">
            ...
            <p className="text-sm font-black text-slate-700">
              {stability_metrics?.cv?.toFixed(1)}%
            </p>
          </div>
          <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100 hover:bg-white transition-all cursor-default group">
            ...
            <p className="text-sm font-black text-emerald-700">
              {stability_metrics?.peak_to_baseline_ratio?.toFixed(2)}x
            </p>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-end gap-1.5 h-12 px-2">
            {formattedDowData.map((item, idx) => {
              const maxVal = Math.max(...formattedDowData.map(d => d.avg_value));
              const heightPct = (item.avg_value / maxVal) * 100;
              return (
      ...
      <div className="px-6 py-3 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-[9px] text-slate-400 flex items-center gap-1">
            ...
            Accuracy: {metadata.accuracy_pct ?? 100}%
          </span>
