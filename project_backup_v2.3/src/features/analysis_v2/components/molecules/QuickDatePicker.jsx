 ...
 */
const QuickDatePicker = ({ label, type = 'start' }) => {
  ...
  const accuracy = normalizationStatus?.accuracy_pct ?? null;
  ...
  const handleApply = () => {
    ...
    // Gunakan 00:00 untuk start, 23:59 untuk end
  ...
  return (
    <div className="relative">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block px-0.5">
        {label}
      </label>
      ...
      {/* Dropdown Panel */}
      {showPicker && !isLocked && (
        <div className="absolute z-50 mt-2 w-full min-w-[280px] bg-white rounded-2xl border border-slate-200 shadow-xl p-4 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="space-y-4">
            {/* Cascading Selects */}
            <div className="grid grid-cols-3 gap-2">
              {/* Year */}
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase ml-1">Tahun</span>
                <select 
                  value={tempDate.year}
                  onChange={(e) => {
              ...
              {/* Month */}
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase ml-1">Bulan</span>
                <select 
              ...
              {/* Day */}
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase ml-1">Tanggal</span>
                <select 
                  value={tempDate.day}
