...
const Reports = () => {
  ...
  React.useEffect(() => {
    if (boards && boards.length > 0 && !selectedBoard) {
      setSelectedBoard(boards[0].boardId);
    }
  ...
  const { 
    ...
    queryKey: ['reports', selectedBoard === 'all' ? 'all' : selectedBoard, activeTab, period, startTime, endTime, clientLimit, selectedBoard === 'all' ? (boards || []).map(b => b.boardId) : null],
    queryFn: async () => {
              ...
              arrays.forEach(arr => {
                (arr || []).forEach(item => {
                  ...
                  const prev = map.get(key) || { 
                    ...
                    cpuSum: 0, cpuCnt: 0, maxCpuLoad: 0 
                  };
                  ...
                  if (item.avgCpuLoad != null) {
                    prev.cpuSum += Number(item.avgCpuLoad || 0);
                    prev.cpuCnt += 1;
                  }
                  prev.maxCpuLoad = Math.max(prev.maxCpuLoad, Number(item.maxCpuLoad || 0));
                  map.set(key, prev);
                });
              });
              result = Array.from(map.values()).map(v => ({
                logDate: v.logDate,
                avgDownload: v.sumAvgDownload,
                avgUpload: v.sumAvgUpload,
                totalDownloadBytes: v.totalDownloadBytes,
                totalUploadBytes: v.totalUploadBytes,
                avgCpuLoad: v.cpuCnt ? v.cpuSum / v.cpuCnt : 0,
                maxCpuLoad: v.maxCpuLoad,
              }));
            } else {
      ...
      const sortedData = [...result].sort((a, b) => {
        const dateA = new Date(a.logDate || a.logMonth || a.logTime);
        const dateB = new Date(b.logDate || b.logMonth || b.logTime);
        return dateA - dateB;
      });
  ...
  const {
    ...
    queryKey: ['reports', 'interfaces', selectedBoard === 'all' ? 'all' : selectedBoard, startTime, endTime, 'summary', selectedBoard === 'all' ? (boards || []).map(b => b.boardId) : null],
    queryFn: async () => {
      if (!selectedBoard) return [];
      if (selectedBoard === 'all') {
        const ids = (boards || []).map(b => b.boardId).filter(Boolean);
        if (ids.length === 0) return [];
        const arrays = await Promise.all(ids.map(id => getInterfaceReports(id, 100, startTime || null, endTime || null)));
        const merged = arrays.flat();
        return [...merged].sort((a, b) => {
          const dateA = new Date(a.logDate || a.logMonth);
          const dateB = new Date(b.logDate || b.logMonth);
          return dateA - dateB;
        });
      }
      const result = await getInterfaceReports(selectedBoard, 100, startTime || null, endTime || null);
  ...
  const {
    ...
    queryKey: ['reports', 'pppoe', selectedBoard === 'all' ? 'all' : selectedBoard, startTime, endTime, 'summary', selectedBoard === 'all' ? (boards || []).map(b => b.boardId) : null],
    queryFn: async () => {
      if (!selectedBoard) return [];
      if (selectedBoard === 'all') {
        const ids = (boards || []).map(b => b.boardId).filter(Boolean);
      ...
      const result = await getPPPoEReports(selectedBoard, 100, startTime || null, endTime || null);
      return [...result].sort((a, b) => {
        const dateA = new Date(a.logDate || a.logMonth);
        const dateB = new Date(b.logDate || b.logMonth);
        return dateA - dateB;
  ...
  const {
    ...
    queryKey: ['reports', 'hotspot', selectedBoard === 'all' ? 'all' : selectedBoard, startTime, endTime, 'summary', selectedBoard === 'all' ? (boards || []).map(b => b.boardId) : null],
    queryFn: async () => {
      if (!selectedBoard) return [];
      if (selectedBoard === 'all') {
        const ids = (boards || []).map(b => b.boardId).filter(Boolean);
      ...
      const result = await getHotspotReports(selectedBoard, 100, startTime || null, endTime || null);
      return [...result].sort((a, b) => {
        const dateA = new Date(a.logDate || a.logMonth);
        const dateB = new Date(b.logDate || b.logMonth);
        return dateA - dateB;
  ...
  const nameOptions = React.useMemo(() => {
    ...
    if (activeTab === 'interfaces') {
      return Array.from(new Set(reportData.map(r => r.interfaceName).filter(Boolean))).sort((a, b) => String(a).localeCompare(String(b)));
    }
    if (activeTab === 'pppoe') {
      return Array.from(new Set(reportData.map(r => r.pppoeUsername).filter(Boolean))).sort((a, b) => String(a).localeCompare(String(b)));
    }
  ...
  const filteredReportData = React.useMemo(() => {
    ...
    if (activeTab === 'interfaces') {
      return (reportData || []).filter(r => String(r.interfaceName || '').toLowerCase().includes(term));
    }
    if (activeTab === 'pppoe') {
      return (reportData || []).filter(r => String(r.pppoeUsername || '').toLowerCase().includes(term));
    }
    if (activeTab === 'hotspot') {
      ...
      if (activeTab === 'clients') {
        if (clientsTableMode === 'pivot') {
          ...
          (reportData || []).forEach(row => {
            const d = new Date(row.logTime || row.logDate || row.logMonth);
            if (isNaN(d.getTime())) return;
          ...
          headers = ['Log Time', 'PPPoE', 'Hotspot', 'Total', 'statId', 'boardId'];
          rows = (reportData || []).map(row => {
            const time = new Date(row.logTime || row.logDate || row.logMonth).toLocaleString();
            const p = Number(row.totalPppoe || 0);
            const h = Number(row.totalHotspot || 0);
            const t = Number(row.totalActive || p + h);
            return [time, p, h, t, row.statId ?? '', row.boardId ?? ''];
          });
          base = `clients_raw`;
        }
      } else if (activeTab === 'interfaces') {
        if (interfacesTableMode === 'pivot') {
          ...
          (filteredReportData || []).forEach(r => {
            const name = r.interfaceName || 'unknown';
            const date = r.displayDate || new Date(r.logDate || r.logMonth || r.logTime).toLocaleDateString();
            const key = `${name}__${date}`;
            const day = byNameDay.get(key) || { name, dl: 0, ul: 0 };
            day.dl += Number(r.totalRxBytes || 0);
            day.ul += Number(r.totalTxBytes || 0);
          ...
          rows = (filteredReportData || []).map(row => {
            const date = row.displayDate || new Date(row.logDate || row.logMonth || row.logTime).toLocaleDateString();
            const dl = (Number(row.totalRxBytes || 0) / div).toFixed(2);
            const ul = (Number(row.totalTxBytes || 0) / div).toFixed(2);
            return [date, row.interfaceName || 'unknown', dl, ul];
          });
        ...
        if (pppoeTableMode === 'pivot') {
          const byNameDay = new Map();
          (filteredReportData || []).forEach(r => {
            const name = r.pppoeUsername || r.username || 'unknown';
            const date = r.displayDate || new Date(r.logDate || r.logMonth || r.logTime).toLocaleDateString();
            const key = `${name}__${date}`;
            const day = byNameDay.get(key) || { name, dl: 0, ul: 0 };
            day.dl += Number(r.downloadBytes || r.dailyDownload || 0);
            day.ul += Number(r.uploadBytes || r.dailyUpload || 0);
            byNameDay.set(key, day);
          });
          const byName = new Map();
          Array.from(byNameDay.values()).forEach(day => {
          ...
          rows = (filteredReportData || []).map(row => {
            const date = row.displayDate || new Date(row.logDate || row.logMonth || row.logTime).toLocaleDateString();
            const dl = (Number(row.downloadBytes || 0) / div).toFixed(2);
            const ul = (Number(row.uploadBytes || 0) / div).toFixed(2);
            return [date, row.pppoeUsername || 'unknown', dl, ul];
          });
        ...
        if (hotspotTableMode === 'pivot') {
          const byNameDay = new Map();
          (filteredReportData || []).forEach(r => {
            const name = r.username || r.hotspotUsername || 'unknown';
            const date = r.displayDate || new Date(r.logDate || r.logMonth || r.logTime).toLocaleDateString();
            const key = `${name}__${date}`;
            const day = byNameDay.get(key) || { name, dl: 0, ul: 0 };
            day.dl += Number(r.dailyDownload || r.downloadBytes || 0);
            day.ul += Number(r.dailyUpload || r.uploadBytes || 0);
            byNameDay.set(key, day);
          });
          const byName = new Map();
          Array.from(byNameDay.values()).forEach(day => {
          ...
          rows = (filteredReportData || []).map(row => {
            const date = row.displayDate || new Date(row.logDate || row.logMonth || row.logTime).toLocaleDateString();
            const dl = (Number(row.dailyDownload || 0) / div).toFixed(2);
            const ul = (Number(row.dailyUpload || 0) / div).toFixed(2);
  ...
  const interfaceTopData = React.useMemo(() => {
    ...
    (filteredReportData || []).forEach(row => {
      const name = row.interfaceName || 'unknown';
      const prev = map.get(name) || { name, downloadValue: 0, uploadValue: 0, totalValue: 0 };
      const dl = Number(row.totalRxBytes || 0) / div;
      const ul = Number(row.totalTxBytes || 0) / div;
  ...
  const pppoeTopData = React.useMemo(() => {
    ...
    const div = usageUnit === 'TB' ? 1024 ** 4 : usageUnit === 'GB' ? 1024 ** 3 : 1024 ** 2;
    (filteredReportData || []).forEach(row => {
      const name = row.pppoeUsername || 'unknown';
      const prev = map.get(name) || { name, downloadValue: 0, uploadValue: 0, totalValue: 0 };
      const dl = Number(row.downloadBytes || 0) / div;
      const ul = Number(row.uploadBytes || 0) / div;
      prev.downloadValue += dl;
      prev.uploadValue += ul;
      prev.totalValue = prev.downloadValue + prev.uploadValue;
      map.set(name, prev);
  ...
  const hotspotTopData = React.useMemo(() => {
    ...
    (filteredReportData || []).forEach(row => {
      ...
      const prev = map.get(name) || { name, downloadValue: 0, uploadValue: 0, totalValue: 0 };
      const dl = Number(row.dailyDownload || 0) / div;
      const ul = Number(row.dailyUpload || 0) / div;
      prev.downloadValue += dl;
      prev.uploadValue += ul;
      prev.totalValue = prev.downloadValue + prev.uploadValue;
      map.set(name, prev);
  ...
  const latestClientCounts = React.useMemo(() => {
    ...
    return {
      pppoe: Number(last.totalPppoe || 0),
      hotspot: Number(last.totalHotspot || 0),
    };
  ...
  const clientsChartData = React.useMemo(() => {
    ...
    const rows = Array.from(byDate.values()).map(r => {
      if (clientsPivotAgg === 'avg') {
        const p = r.cnt ? Math.round(r.sumP / r.cnt) : 0;
        const h = r.cnt ? Math.round(r.sumH / r.cnt) : 0;
        return { date: r.date, totalPppoe: p, totalHotspot: h };
      }
      return { date: r.date, totalPppoe: r.maxP, totalHotspot: r.maxH };
    }).sort((a, b) => {
  ...
  const renderTrafficOverview = () => {
    const downloadKey = period === 'monthly' ? 'totalDownloadBytes' : 'avgDownload';
    const uploadKey = period === 'monthly' ? 'totalUploadBytes' : 'avgUpload';
    return (
      ...
      <SizedContainer heightClass={chartHeight}>
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={300}>
          <LineChart data={reportData}>
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="avgCpuLoad" name="CPU Load (%)" stroke="#ef4444" />
            <Line type="monotone" dataKey="maxCpuLoad" name="Max CPU (%)" stroke="#b91c1c" strokeDasharray="5 5" />
          </LineChart>
        </ResponsiveContainer>
      </SizedContainer>
    </div>
      ...
      <div className={`bg-white ${cardPadding} rounded-lg shadow mb-6`}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-1">Date Range (ISO 8601)</label>
                    ...
                    <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={300}>
                      <BarChart data={(function(){
                        ...
                        (pppoeSummary || []).forEach(row => {
                          const name = row.pppoeUsername || 'unknown';
                          const prev = map.get(name) || { name, downloadValue: 0, uploadValue: 0, totalValue: 0 };
                          const dl = Number(row.downloadBytes || row.dailyDownload || 0) / div;
                          const ul = Number(row.uploadBytes || row.dailyUpload || 0) / div;
                          prev.downloadValue += dl;
                          prev.uploadValue += ul;
                          prev.totalValue = prev.downloadValue + prev.uploadValue;
                          map.set(name, prev);
                ...
                <SizedContainer heightClass={chartHeight}>
                  <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={300}>
                    ...
                    <XAxis dataKey="date" />
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="totalPppoe" name="PPPoE Clients" fill="#3b82f6" stackId={clientsChartStacked ? 'clients' : undefined} />
                      <Bar dataKey="totalHotspot" name="Hotspot Clients" fill="#f59e0b" stackId={clientsChartStacked ? 'clients' : undefined} />
                    </BarChart>
                  </ResponsiveContainer>
                </SizedContainer>
              </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Hotspot</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">statId</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">boardId</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {(reportData || []).map((row, idx) => (
                        <tr key={idx}>
                          <td className="px-6 py-3 text-sm text-gray-900">{new Date(row.logTime || row.logDate || row.logMonth).toLocaleString()}</td>
                          <td className="px-6 py-3 text-sm text-gray-900">{Number(row.totalPppoe || 0)}</td>
                          <td className="px-6 py-3 text-sm text-gray-900">{Number(row.totalHotspot || 0)}</td>
                          <td className="px-6 py-3 text-sm text-gray-900">{Number(row.totalActive || (Number(row.totalPppoe || 0) + Number(row.totalHotspot || 0)))}</td>
                          <td className="px-6 py-3 text-sm text-gray-900">{row.statId}</td>
                          <td className="px-6 py-3 text-sm text-gray-900">{row.boardId}</td>
                        </tr>
                      ))}
                      {(reportData || []).length === 0 && (
                        <tr>
              ...
              <div className="space-y-6">
                ...
                {interfacesTableMode === 'pivot' ? (
                  ...
                  renderTable([
                    ...
                    { label: 'Interface', key: 'interfaceName' },
                    { label: `Total Download (${usageUnit})`, key: 'totalRxBytes', render: (row) => {
                      const div = usageUnit === 'TB' ? 1024 ** 4 : usageUnit === 'GB' ? 1024 ** 3 : 1024 ** 2;
                      return `${(Number(row.totalRxBytes || 0) / div).toFixed(2)} ${usageUnit}`;
                    }},
                    { label: `Total Upload (${usageUnit})`, key: 'totalTxBytes', render: (row) => {
                      const div = usageUnit === 'TB' ? 1024 ** 4 : usageUnit === 'GB' ? 1024 ** 3 : 1024 ** 2;
                      return `${(Number(row.totalTxBytes || 0) / div).toFixed(2)} ${usageUnit}`;
                    }},
                  ], filteredReportData)
                )}
              </div>
            ...
            {activeTab === 'pppoe' && (
              <div className="space-y-6">
                ...
                {pppoeTableMode === 'pivot' ? (
                  ...
                  renderTable([
                    { label: 'Date', key: 'displayDate' },
                    { label: 'Username', key: 'pppoeUsername' },
                    { label: `Total Download (${usageUnit})`, key: 'downloadBytes', render: (row) => {
                      const div = usageUnit === 'TB' ? 1024 ** 4 : usageUnit === 'GB' ? 1024 ** 3 : 1024 ** 2;
                      return `${(Number(row.downloadBytes || 0) / div).toFixed(2)} ${usageUnit}`;
                    }},
                    { label: `Total Upload (${usageUnit})`, key: 'uploadBytes', render: (row) => {
                      const div = usageUnit === 'TB' ? 1024 ** 4 : usageUnit === 'GB' ? 1024 ** 3 : 1024 ** 2;
                      return `${(Number(row.uploadBytes || 0) / div).toFixed(2)} ${usageUnit}`;
                    }},
                  ], filteredReportData)
                )}
              </div>
              ...
              <div className="space-y-6">
                ...
                {hotspotTableMode === 'pivot' ? (
                  ...
                  renderTable([
                    ...
                    { label: `Total Download (${usageUnit})`, key: 'dailyDownload', render: (row) => {
                      const div = usageUnit === 'TB' ? 1024 ** 4 : usageUnit === 'GB' ? 1024 ** 3 : 1024 ** 2;
                      return `${(Number(row.dailyDownload || 0) / div).toFixed(2)} ${usageUnit}`;
                    }},
                    { label: `Total Upload (${usageUnit})`, key: 'dailyUpload', render: (row) => {
                      const div = usageUnit === 'TB' ? 1024 ** 4 : usageUnit === 'GB' ? 1024 ** 3 : 1024 ** 2;
                      return `${(Number(row.dailyUpload || 0) / div).toFixed(2)} ${usageUnit}`;
                    }},
                  ], filteredReportData)
                )}
              </div>
