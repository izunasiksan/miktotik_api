...
const ZTPQueue = () => {
  ...
  const approveMutation = useMutation({
    mutationFn: ({ id, site_group }) => approveZTPDevice(id, { site_group }),
    onSuccess: () => {
  ...
  const handleApprove = (device) => {
    ...
    approveMutation.mutate({ id: device.ztp_id, site_group: site });
  };
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {queue.map((device) => (
              <tr key={device.ztp_id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {format(new Date(device.requested_at), 'MMM d, HH:mm:ss')}
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex justify-end gap-2">
                  ...
                  <button 
                    onClick={() => handleReject(device.ztp_id)}
