    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            if await check_blacklist(client_ip):
                ...
                raise HTTPException(status_code=403, detail="Access Denied: IP Blacklisted due to excessive violations.")
        except Exception as e:
            # Fail-open if Redis is down
            print(f"Blacklist check failed (Redis error): {e}. Proceeding as fail-open.")
            
        return await call_next(request)

class IPWhitelistMiddleware(BaseHTTPMiddleware):
    def __init__(self, app, allowed_ips: list[str], protected_paths: list[str] | None = None):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self.protected_paths:
            ...
            is_protected = any(current_path.startswith(path) for path in self.protected_paths)
            if not is_protected:
                return await call_next(request)
        ...
        if client_ip not in self.allowed_ips:
             raise HTTPException(status_code=403, detail="Access Denied: IP Address not allowed for this resource.")
