...
async def get_dashboard_summary(
    ...
    online_boards = await db.scalar(select(func.count(MikrotikBoard.board_id)).where(MikrotikBoard.is_online.is_(True)))
    offline_boards = await db.scalar(select(func.count(MikrotikBoard.board_id)).where(MikrotikBoard.is_online.is_(False)))
    maintenance_boards = await db.scalar(select(func.count(MikrotikBoard.board_id)).where(MikrotikBoard.is_maintenance.is_(True)))
