...
async def aggregate_daily_stats(
    ...
    logger.info(f"Starting optimized daily aggregation for range: {start_time} - {end_time}")
...
async def aggregate_monthly_stats(
    ...
    logger.info(f"Starting optimized monthly aggregation for range: {start_time} - {end_time}")
...
async def run_monthly_aggregation_job():
    ...
    first_this_month = today.replace(day=1)
    start_time = datetime.combine(first_this_month - timedelta(days=1), datetime.min.time()).replace(day=1)
    # end_time adalah akhir hari kemarin
    end_time = datetime.combine(first_this_month - timedelta(seconds=1), datetime.max.time())
