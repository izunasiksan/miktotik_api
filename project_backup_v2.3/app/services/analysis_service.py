...
async def create_scoped_dataset(
    ...
    # V2.3: Added unique suffix to prevent race conditions in connection pool
    ...
    # P3: Materialization Optimization for long range (> 365 days)
    # If range > 365 days, use daily summaries instead of raw stats for performance
    ...
    if use_summary:
        ...
        query = text(f"""
            ...
            SELECT 
                ...
                avg_download as rx,
                avg_upload as tx,
                100.0 as acc_traffic, -- Summaries are considered 100% accurate or pre-normalized
                avg_cpu_load as cpu,
        ...
        # Create temp table combining traffic and resource from raw stats
...
async def get_trend_analysis(
    ...
    # Calculate average accuracy from temp table
    query_acc = text(f"SELECT avg(acc_traffic) as acc_t, avg(acc_resource) as acc_r FROM {temp_table}")
    res_acc = await db.execute(query_acc)
    acc_row = res_acc.fetchone()
    avg_acc = (float(acc_row[0] or 100.0) + float(acc_row[1] or 100.0)) / 2 if acc_row else 100.0
