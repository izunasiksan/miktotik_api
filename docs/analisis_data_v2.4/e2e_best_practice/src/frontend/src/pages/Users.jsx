...
import { getUsers, createUser, updateUser, deleteUser, getMasterRoles } from '../services/api.js';
...
const Users = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        password: '',
        fullName: '',
        roleId: 2, // 3NF: Use roleId instead of role string
        isActive: true
    });
    ...
    useEffect(() => {
        const fetchRoles = async () => {
            try {
                const rolesData = await getMasterRoles();
                setRoles(rolesData);
            } catch (error) {
    ...
    const handleSubmit = (e) => {
        ...
        if (currentUserEdit) {
            ...
            updateMutation.mutate({ id: currentUserEdit.userId, data: updateData });
        } else {
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {users.map(user => (
                            <tr key={user.userId} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-gray-500">{user.fullName}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full items-center gap-1 ${user.roleRel?.roleName === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'}`}>
                                        <Shield className="w-3 h-3" />
                                        {user.roleRel?.roleName || 'Unknown'}
                                    </span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full items-center gap-1 ${user.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                        ...
                                        <div className="mt-4">
                                            <form onSubmit={handleSubmit} id="user-form">
                                                <div className="mb-4">
                                                    <label htmlFor="user-username" className="block text-gray-700 text-sm font-bold mb-2">Username</label>
                                                    <input
                                                        id="user-username"
                                                        type="text"
                                                        name="username"
                                                ...
                                                <div className="mb-4">
                                                    <label htmlFor="user-password" title="Password" className="block text-gray-700 text-sm font-bold mb-2">
                                                ...
                                                <div className="mb-4">
                                                    <label htmlFor="user-fullname" className="block text-gray-700 text-sm font-bold mb-2">Nama Lengkap</label>
                                                    <input
                                                        id="user-fullname"
                                                        type="text"
                                                        name="fullName"
                                                ...
                                                <div className="mb-4">
                                                    <label htmlFor="user-role-select" className="block text-gray-700 text-sm font-bold mb-2">Role</label>
                                                    <select
                                                        id="user-role-select"
                                                        name="roleId"
                                                ...
                                                <div className="mb-6">
                                                    <label htmlFor="user-active-checkbox" className="flex items-center cursor-pointer">
                                                        <input
