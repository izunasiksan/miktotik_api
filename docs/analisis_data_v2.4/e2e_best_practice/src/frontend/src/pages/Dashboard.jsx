...
const Dashboard = () => {
  ...
  const filteredBoards = boards.filter(board => {
    if (deviceFilter === 'online') return board.isOnline && !board.isMaintenance;
    if (deviceFilter === 'offline') return !board.isOnline && !board.isMaintenance;
    if (deviceFilter === 'maintenance') return board.isMaintenance;
    return true;
  });

  const publicBoards = (boards || []).filter(b => b.isPublicReview !== false);
  const sites = (() => {
    const set = new Set();
    (boards || []).forEach(b => {
      if (b.site?.siteName) set.add(b.site.siteName);
    });
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Link to="/boards?status=maintenance" className="block group">
          <StatCard 
            title="Maintenance" 
            value={safeStats.maintenanceBoards} 
            icon={Activity} 
            color="bg-amber-500" 
          />
        </Link>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <Card title="Navigasi" subtitle="Akses cepat berdasarkan data">
          ...
          <div className="mt-6">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Sites</p>
            {sites.length === 0 ? (
              <p className="text-xs text-slate-400">Tidak ada data site.</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {sites.map(siteName => (
                  <Link key={siteName} to={`/boards?q=${encodeURIComponent(siteName)}`}>
