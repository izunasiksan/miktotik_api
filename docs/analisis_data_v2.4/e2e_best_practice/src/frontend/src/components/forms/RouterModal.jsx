...
import { getMasterSites, getMasterModels } from '../../services/api';
...
const RouterModal = ({ isOpen, onClose, onSave, initialData }) => {
  const [formData, setFormData] = useState({
    ...
    mac_address: '',
    port_api: 8728,
    site_id: 1, // 3NF: Use site_id instead of site_group
    model_id: 1, // 3NF: Added model_id
    username_mikrotik: '',
    password_mikrotik: '',
    is_monitor: true,
  });
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        const [sitesData, modelsData] = await Promise.all([
          getMasterSites(),
          getMasterModels()
        ]);
        setSites(sitesData);
    ...
    const id = setTimeout(() => {
      if (initialData) {
        setFormData({
          ...
          site_id: initialData.site_id || (sites.length > 0 ? sites[0].site_id : 1),
          model_id: initialData.model_id || (models.length > 0 ? models[0].model_id : 1),
        ...
        setFormData({
          ...
          model_id: models.length > 0 ? models[0].model_id : 1,
          username_mikrotik: '',
          password_mikrotik: '',
          is_monitor: true,
        });
  ...
  }, [initialData, isOpen, sites, models]);
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...
      [name]: type === 'checkbox' ? checked : 
              (name === 'site_id' || name === 'model_id' || name === 'port_api' ? Number(value) : value),
    }));
  };
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={initialData ? 'Edit Router' : 'Add New Router'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Board Model</label>
            <select
              name="model_id"
