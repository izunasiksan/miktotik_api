...
const InsightCard = () => {
  ...
  const healthMetrics = React.useMemo(() => {
    ...
    const stability = analysisData.stage4?.stabilityMetrics?.stabilityScore ?? 100;
    const utilization = analysisData.stage6?.healthScore ?? 100; // Stage 6 already provides a score
    const anomalyPenalty = analysisData.stage5?.summary?.penaltyScore ?? 0;
    ...
    const accuracy = analysisData.stage2?.metadata?.accuracyPct ?? 100;
  ...
  const topSignals = React.useMemo(() => {
    ...
    // Anomalies (Stage 5)
    const activeAnomalies = analysisData.stage5?.activeAnomalies || [];
    ...
    if (forecastSummary && forecastSummary.risk !== 'LOW') {
      signals.push({
        ...
        message: `Penyempitan headroom terdeteksi. TTC: ${forecastSummary.thresholdBreach || 'N/A'}`,
        timestamp: analysisData?.metadata?.processedAt,
        sourceId: 'ForecastEngine',
    ...
    highCorrelations.slice(0, 2).forEach(corr => {
      signals.push({
        id: `corr-${corr.metricA}-${corr.metricB}`,
        type: 'CORRELATION',
        severity: 'INFO',
        title: 'Korelasi Kuat',
        message: `${corr.metricA} & ${corr.metricB} memiliki hubungan ${corr.value > 0 ? 'positif' : 'negatif'} yang signifikan.`,
        timestamp: analysisData?.metadata?.processedAt,
        sourceId: 'CorrelationEngine',
  ...
  const getInsightIcon = (type) => {
    switch (type) {
      ...
      case 'users': return <Database className="w-3.5 h-3.5 text-emerald-600" />;
    ...
    >
      ...
      <div className="pt-2 flex items-center justify-between border-t border-slate-100 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <span className="text-[8px] text-slate-400 font-medium flex items-center gap-1">
          <Database className="w-2.5 h-2.5" /> {insight.sourceId || 'System'}
        </span>
        <span className="text-[8px] text-slate-400 italic">
          {insight.rawTimestamp ? new Date(insight.rawTimestamp).toLocaleTimeString() : 'N/A'}
        </span>
      </div>
    </div>
  );
          ...
          <div className="flex items-center gap-3 text-[10px] text-slate-400 font-medium">
            <span className="flex items-center gap-1">
              <Database className="w-3 h-3" /> Source: Raw Data (SSOT)
                ...
                {analysisData.stage2?.metadata?.sourceGranularity && (
                  <div className="flex items-center gap-1">
                    <Database className="w-3 h-3" /> Granularity: {analysisData.stage2.metadata.sourceGranularity}
                  </div>
                )}
              </div>
        </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg border border-slate-100">
              ...
              <span className="text-[10px] font-bold text-indigo-600">
                {analysisData.stage0?.normalizationMetrics?.completenessScore || 100}%
              </span>
            </div>
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg border border-slate-100">
            ...
            {topSignals.length > 0 ? topSignals.map(signal => (
              <div key={signal.id} className="p-3 bg-white border border-slate-100 rounded-xl shadow-sm space-y-2 hover:border-indigo-200 transition-colors">
                ...
                <div className="flex items-center justify-between pt-1 border-t border-slate-50 text-[8px] text-slate-400">
                  <span className="flex items-center gap-1"><Zap className="w-2.5 h-2.5" /> {signal.sourceId}</span>
      ...
      <div className="pt-6 border-t border-slate-100 flex items-center justify-between text-[9px] text-slate-400 font-medium">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5">
            <Database className="w-3 h-3" /> Audit Reference: {analysisData.stage1?.scopedMetadata?.boardId || 'Global'}
          </span>
          <span className="flex items-center gap-1.5">
            <Clock className="w-3 h-3" /> Processed: {analysisData?.metadata?.processedAt ? new Date(analysisData.metadata.processedAt).toLocaleString() : 'N/A'}
          </span>
          <span className="flex items-center gap-1.5">
