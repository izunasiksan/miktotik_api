 ...
 * Menampilkan status kualitas data (accuracyPct) hasil normalisasi Stage 0.
...
const DataQualityAlert = () => {
  ...
  const { accuracyPct, missingGaps, status } = normalizationStatus;
    ...
    <div className={`p-4 rounded-2xl border flex items-start gap-4 mb-8 shadow-sm transition-all animate-in fade-in slide-in-from-left-4 duration-500 ${colorClass}`}>
      ...
      <div className="flex-1 space-y-1">
        <div className="flex justify-between items-center">
          ...
          <DataQualityBadge accuracyPct={accuracyPct} />
        </div>
        
        <p className="text-xs font-medium opacity-80 leading-relaxed">
          {accuracyPct < 100 
            ? `Dataset mengandung ${missingGaps || 0} gap data. Menggunakan estimasi statistik untuk menjamin kontinuitas pipeline.`
