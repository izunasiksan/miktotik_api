        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between items-center text-[10px]">
            ...
            <span className="text-slate-300">
              [{payload[1]?.payload?.lowerBound?.toFixed(1)} - {payload[1]?.payload?.upperBound?.toFixed(1)}]
            </span>
          </div>
        </div>
      </div>
...
const CapacityForecast = () => {
  ...
  const derivedMetrics = React.useMemo(() => {
    ...
    const { forecastData, capacityMetadata } = analysisData.stage6;
    const capacity = capacityMetadata?.currentCapacity ?? 100; // fallback to 100 if missing
    
    // 1. Calculate Utilization & Headroom for each point
    const enrichedForecast = forecastData?.map(point => ({
      ...point,
      utilization: (point.projectedValue / capacity) * 100,
      headroom: Math.max(0, capacity - point.upperBound)
    })) || [];

    // 2. Derive TTC (Time-To-Capacity)
    // TTC terjadi saat: upperBound >= capacity (Conservative Rule)
    const ttcPoint = enrichedForecast.find(p => p.upperBound >= capacity);
    ...
    const currentPoint = enrichedForecast[0] || { projectedValue: 0, upperBound: 0 };
    const currentUtilization = (currentPoint.projectedValue / capacity) * 100;
    const currentHeadroom = Math.max(0, capacity - currentPoint.upperBound);

    return {
      enrichedForecast,
      capacity,
      unit: capacityMetadata?.unit || 'Mbps',
      ttcLabel,
      ttcStatus,
      currentUtilization,
      currentHeadroom,
      sourceId: capacityMetadata?.sourceId || 'Unknown',
      accuracy: analysisData.stage6.scoringMetrics?.confidenceScore ?? 0
    };
  }, [analysisData]);
  ...
  const { healthScore } = analysisData.stage6;
    ...
    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex flex-col items-end gap-1">
          <div className={`px-3 py-1.5 rounded-xl border text-xs font-bold flex items-center gap-1.5 shadow-sm ${getHealthColor(healthScore)}`}>
