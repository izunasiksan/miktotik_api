...
const HotspotAnalytics = ({ boardId }) => {
  ...
  const { data: stats = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['hotspot-reports', boardId, startTime, endTime],
    queryFn: () => getHotspotReports(boardId, 100, startTime || null, endTime || null),
  });
  ...
  const chartData = stats.map(item => ({
    name: item.logDate ? new Date(item.logDate).toLocaleDateString() : 'Unknown',
    users: item.username ? 1 : 0, // Count unique users per day
    traffic: ((Number(item.dailyDownload || 0) + Number(item.dailyUpload || 0)) / (1024 * 1024)).toFixed(2)
  }));
  ...
  const aggregatedData = chartData.reduce((acc, item) => {
    ...
    if (existing) {
      ...
      existing.traffic = (Number(existing.traffic) + Number(item.traffic)).toFixed(2);
    } else {
      acc.push({ ...item });
    }
    return acc;
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
            <div className="flex flex-col">
                <label htmlFor="hotspot-start-time" className="text-[10px] font-semibold text-gray-500 mb-0.5">Start Time (ISO)</label>
                <input 
                    id="hotspot-start-time"
                    name="startTime"
                    type="datetime-local" 
            ...
            <div className="flex flex-col">
                <label htmlFor="hotspot-end-time" className="text-[10px] font-semibold text-gray-500 mb-0.5">End Time (ISO)</label>
                <input 
                    id="hotspot-end-time"
                    name="endTime"
                    type="datetime-local" 
