...
import { getHotspotUsers, kickHotspotUser } from '../../../services/api.js';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { toast } from 'react-hot-toast';
import LoadingSpinner from '../../../components/atoms/LoadingSpinner.jsx';
import { Wifi, WifiOff, RefreshCw } from 'lucide-react';

const HotspotMonitor = ({ boardId }) => {
  const queryClient = useQueryClient();
  const [searchTerm, setSearchTerm] = useState('');

  const { data: users = [], isLoading, isError, error, refetch } = useQuery({
    queryKey: ['hotspot-users', boardId],
    queryFn: () => getHotspotUsers(boardId),
    refetchInterval: 15000, // Auto-refresh every 15 seconds
  });

  const disconnectMutation = useMutation({
    mutationFn: (username) => kickHotspotUser(boardId, username),
    onSuccess: (_, username) => {
      queryClient.invalidateQueries({ queryKey: ['hotspot-users', boardId] });
    ...
    <div className="bg-white shadow rounded-lg p-6 mt-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-2">
            <label htmlFor="hotspot-search-input" className="sr-only">Search user</label>
            <input 
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredUsers.map((user) => (
              <tr key={user.id || user.user}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button 
                    onClick={() => handleDisconnect(user.user)}
            ...
            {filteredUsers.length === 0 && (
                <tr>
                    <td colSpan="6" className="text-center py-4 text-gray-500">No active sessions found.</td>
                </tr>
            )}
          </tbody>
        </table>
