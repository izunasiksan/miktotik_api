...
async def run_daily_backups():
    ...
    async with AsyncSessionLocal() as db:
        ...
        stmt = select(MikrotikBoard).where(
            MikrotikBoard.is_monitor.is_(True), MikrotikBoard.is_maintenance.is_(False)
        )
        ...
        async def backup_single_board(board):
            async with semaphore:
                ...
                try:
                    logger.info(
                        f"⏳ [Backup] Starting backup for {board.board_name} ({board.ip_address})..."
                    )

                    # Create a new session per task to be safe with async concurrency
                    async with AsyncSessionLocal() as task_db:
                        await backup_service.create_backup(
                            task_db, board.board_id, file_name
                        )
                    ...
                    logger.error(
                        f"❌ [Backup] Failed for {board.board_name}: {e.detail}"
                    )
            ...
            for file_path in files:
                try:
                    ...
                    if file_time < cutoff_time:
                        ...
                        count += 1
                        logger.info(
                            f"🗑️ [Cleanup] Deleted old backup: {os.path.basename(file_path)}"
                        )
                except Exception as e:
    ...
    logger.info(
        f"🏁 [Scheduler] Cleanup Completed. Deleted {deleted_files_count} files."
    )
    ...
    if report_data["failed"]:
        ...
        for fail in report_data["failed"][
            :10
        ]:  # Limit 10 items to prevent huge message
            message += f"• {fail}\n"
        if failed_count > 10:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if not admin_ids:
                logger.warning(
                    "⚠️ [Telegram] No Admin ID found in DB or Settings. Skipping report."
                )
            else:
                for admin_id in admin_ids:
                    ...
                    await telegram_service.send_message(
                        admin_id, message, token=token_to_use
                    )
        except Exception as e:
