...
from app.models.mikrotik import BoardCredential, MikrotikBoard
...
class AutomationService:
    async def _execute_ssh_command(self, host, port, username, password, command):
        ...
        try:
            async with asyncssh.connect(
                ...
                stderr_text = (
                    (result.stderr or "").strip() if result.exit_status != 0 else None
                )
                return {
    ...
    async def run_mass_config(
        ...
        for board in boards:
            log_entry = AutomationLog(
                job_id=job.job_id, board_id=board.board_id, status="pending"
            )
            db.add(log_entry)
            ...
            try:
                # Credential check
                cred_query = select(BoardCredential).where(
                    BoardCredential.board_id == board.board_id
                )
                ...
                ssh_res = await self._execute_ssh_command(
                    str(getattr(board, "ip_address", "")),
                    int(getattr(board, "port_ssh", 22) or 22),
                    str(getattr(cred, "username_mikrotik", "")),
                    password,
                    command,
                )
    ...
    async def trigger_auto_heal(
        self, db: AsyncSession, board_id: UUID, reason: str, user_id: UUID | None = None
    ) -> Dict[str, Any]:
        """
        ...
        log_entry = AutomationLog(
            job_id=job.job_id, board_id=board_id, status="pending"
        )
        db.add(log_entry)
        ...
        try:
            # Get credentials
            cred_query = select(BoardCredential).where(
                BoardCredential.board_id == board_id
            )
            cred_res = await db.execute(cred_query)
            cred = cred_res.scalar_one_or_none()
            ...
            return {
                "status": "success",
                "message": f"Reboot command sent to {board.board_name}",
            }
    ...
    async def register_ztp_device(
        ...
        # Check Queue
        q_query = select(ZTPQueue).where(
            ZTPQueue.mac_address == mac_address, ZTPQueue.status == "pending"
        )
    ...
    async def get_ztp_queue(
        self, db: AsyncSession, skip: int = 0, limit: int = 100
    ) -> List[ZTPQueue]:
        """
        Mendapatkan daftar antrian ZTP yang masih pending.
        """
        query = (
            select(ZTPQueue)
            .where(ZTPQueue.status == "pending")
            .offset(skip)
            .limit(limit)
            .order_by(ZTPQueue.requested_at.desc())
        )
        result = await db.execute(query)
        return list(result.scalars().all())

    async def approve_ztp_device(
        self,
        db: AsyncSession,
        ztp_id: UUID,
        site_id: int,
        ...
        """
        Menyetujui perangkat di antrian ZTP dan mendaftarkannya sebagai MikrotikBoard resmi.
        """
        # 1. Cari di Queue
        query = select(ZTPQueue).where(ZTPQueue.ztp_id == ztp_id)
        result = await db.execute(query)
        ztp_item = result.scalar_one_or_none()

        if not ztp_item:
            raise Exception("ZTP Request not found")
        if ztp_item.status != "pending":
            raise Exception(f"ZTP Request already {ztp_item.status}")

        # 2. Create MikrotikBoard
        new_board = MikrotikBoard(
            board_name=f"ZTP-{ztp_item.identity}-{ztp_item.mac_address[-4:]}",
            mikrotik_identity=ztp_item.identity,
            model_id=model_id,
            site_id=site_id,
            mac_address=ztp_item.mac_address,
            ip_address=ztp_item.ip_address,
            port_api=8728,
            is_monitor=True,
        )
        db.add(new_board)
        await db.flush()  # Get board_id

        # 3. Create Credentials
        new_cred = BoardCredential(
            board_id=new_board.board_id,
            username_mikrotik=ztp_item.temp_username,
            password_mikrotik_encrypted=ztp_item.temp_password,
        )
        db.add(new_cred)

        # 4. Update ZTP Queue Status
        ztp_item.status = "approved"
        ztp_item.approved_at = datetime.now()
        ztp_item.board_id = new_board.board_id
    ...
    async def reject_ztp_device(self, db: AsyncSession, ztp_id: UUID) -> bool:
        """
        Menolak perangkat di antrian ZTP.
        """
        query = select(ZTPQueue).where(ZTPQueue.ztp_id == ztp_id)
        result = await db.execute(query)
        ztp_item = result.scalar_one_or_none()
    ...
    async def apply_qos_policy(
        ...
        log = AutomationLog(
            ...
            output=(
                ssh_res["output"]
                if status == "success"
                else ssh_res["error"] or "Command Failed"
            ),
        )
        db.add(log)
        await db.commit()
