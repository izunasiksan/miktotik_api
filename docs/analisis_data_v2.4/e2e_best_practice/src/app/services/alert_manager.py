...
from app.models.mikrotik import BoardEvent, MikrotikBoard
...
class AlertManager:
    ...
    MAX_FAILURE_COUNT = (
        3  # Rule 1.3: Jika gagal berturut-turut > 3 kali, status router update OFFLINE
    )

    async def check_health(
        self, session: AsyncSession, board: MikrotikBoard, metrics: Dict[str, Any]
    ):
        """
        ...
        if getattr(board, "is_maintenance", False):
            logger.info(
                f"Board {board.board_name} is in maintenance. Alerts suppressed."
            )
            return
    ...
    async def _handle_offline_event(self, session: AsyncSession, board: MikrotikBoard):
        ...
        self._failure_counts[board_id_str] = (
            self._failure_counts.get(board_id_str, 0) + 1
        )
        count = self._failure_counts[board_id_str]

        logger.warning(
            f"Board {board.board_name} failure detected. Count: {count}/{self.MAX_FAILURE_COUNT}"
        )
        ...
        if count >= self.MAX_FAILURE_COUNT:
            ...
            if getattr(board, "is_online", False):
                logger.warning(
                    f"Board {board.board_name} confirmed OFFLINE after {count} consecutive failures."
                )

                # Update Board Status in DB
                stmt = (
                    update(MikrotikBoard)
                    .where(MikrotikBoard.board_id == board.board_id)
                ...
                await self._log_event(
                    ...
                    "critical",
                    "Device Offline",
                    f"Device detected offline for {count} consecutive polling cycles.",
                )
    ...
    async def _handle_recovery_event(self, session: AsyncSession, board: MikrotikBoard):
        ...
        if not is_online_flag:
            ...
            # Log Event
            await self._log_event(
                session, board, "info", "Device Recovered", "Connection restored"
            )
    ...
    async def _trigger_alert(
        self, session: AsyncSession, board: MikrotikBoard, level: str, message: str
    ):
