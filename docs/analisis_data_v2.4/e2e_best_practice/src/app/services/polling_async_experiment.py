...
async def poll_board_ssh(
    host: str, username: str, password: str, port: int = 22, timeout: int = 15
    ...
    try:
        async with asyncssh.connect(
            ...
            cmd_counts = (
                ":put [/ip hotspot active print count-only]; "
                ":put [/ppp active print count-only];"
            )
            cmd_interfaces = (
                ':foreach i in=[/interface find] do={ '
                ':local n [/interface get $i name]; '
                ':local r [/interface get $i rx-byte]; '
                ':local t [/interface get $i tx-byte]; '
                ':local ty [/interface get $i type]; '
                ':put ($n . "," . $r . "," . $t . "," . $ty) };'
            )
            ...
            if isinstance(stdout_val, memoryview):
                stdout_val = stdout_val.tobytes()
            
            stdout_text = (stdout_val.decode("utf-8", errors="ignore") if isinstance(stdout_val, bytes) else str(stdout_val)).strip()
...
async def _poll_board_task(board_id: Any, sem: asyncio.Semaphore):
    ...
    async with sem, SessionLocal() as session:
        ...
        port = int(getattr(board, "port_ssh", 22) or 22)
        ...
        for iface in data["interfaces"]:
            ...
            # Update memory
            _prev_stats[board_id_str]["interface"][name] = {
                "rx": curr_rx,
                "tx": curr_tx,
                "time": current_time,
            }
