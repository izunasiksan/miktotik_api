...
from app.models.mikrotik import BoardBackup, MikrotikBoard


class BackupService:
    async def create_backup(
        self, db: AsyncSession, board_id: UUID, file_name: str
    ) -> BoardBackup:
        ...
        decrypted_password = decrypt_password(
            str(enc_val) if enc_val is not None else ""
        )
        ...
        try:
            async with asyncssh.connect(
                ...
                result = await conn.run(
                    f'/system/backup/save name="{file_name}"', check=True, timeout=60
                )
            ...
            raise HTTPException(
                status_code=403,
                detail="SSH Permission Denied. Check username/password.",
            )
        except asyncssh.TimeoutError:
            raise HTTPException(
                status_code=504, detail="SSH Connection/Command Timeout."
            )
    ...
    async def get_backups(
        self, db: AsyncSession, board_id: UUID, skip: int = 0, limit: int = 100
    ):
        stmt = (
            select(BoardBackup)
    ...
    async def restore_backup(self, db: AsyncSession, backup_id: UUID):
        ...
        file_location_str = (
            str(file_location_val) if file_location_val is not None else ""
        )
        ...
        try:
            async with asyncssh.connect(
                ...
                try:
                    await conn.run(
                        f'/system/backup/load name="{remote_filename}" password=""',
                        timeout=10,
                    )
        ...
        return {
            "message": f"Restore initiated for {backup.file_name}. Router is rebooting."
        }
