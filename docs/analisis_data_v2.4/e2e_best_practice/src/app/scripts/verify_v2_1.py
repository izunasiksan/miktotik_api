    ...
    async with SessionLocal() as db:
        ...
        for table in tables_to_check:
            res = await db.execute(
                text(
                    f"SELECT column_name FROM information_schema.columns WHERE table_name = '{table}' AND column_name = 'accuracy_pct'"
                )
            )
            if res.fetchone():
        ...
        try:
            ...
            if norm_result["traffic"]:
                ...
                if "accuracy_pct" in sample and "source_id" in sample:
                    logger.info(
                        "Normalization Output: Metadata 'accuracy_pct' and 'source_id' verified."
                    )
                else:
                    logger.error(
                        "Normalization Output: Missing mandatory metadata fields."
                    )
            ...
            if norm_result["resource"]:
                ...
                if "accuracy_pct" in sample and "source_id" in sample:
                    logger.info(
                        "Resource Output: Metadata 'accuracy_pct' and 'source_id' verified."
                    )
                else:
                    logger.error(
                        "Resource Output: Missing mandatory metadata fields in resource."
                    )
