...
class AutomationJob(Base):
    ...
    logs = relationship(
        "AutomationLog", back_populates="job", cascade="all, delete-orphan"
    )
...
class AutomationLog(Base):
    ...
    job_id = Column(
        UUID(as_uuid=True), ForeignKey("automation_jobs.job_id", ondelete="CASCADE")
    )
    board_id = Column(
...
class ZTPQueue(Base):
    ...
    board_id = Column(UUID(as_uuid=True), ForeignKey("mikrotik_boards.board_id"))
    ...
    __table_args__ = (Index("idx_ztp_status", "status"),)
