...
from app.models.user import MasterUser, UserBoardAccess, MasterRole
from app.schemas.user import (
    User,
    UserBoardAccessCreate,
    UserBoardAccessResponse,
    UserCreate,
    UserUpdate,
    MasterRoleResponse,
)
...
async def read_users(
    ...
    """
    Retrieve users.
    V2.3: Mendukung filter userId dengan camelCase alias.
    """
    cache_key = f"users:list:{skip}:{limit}:{user_id}"
    cached_data = await redis_client.get(cache_key)
    if cached_data:
        return json.loads(cached_data)

    # Gunakan Eager Loading (3NF V2.4)
    stmt = select(MasterUser).options(joinedload(MasterUser.role_rel))
    if user_id:
        stmt = stmt.where(MasterUser.user_id == user_id)
    ...
    data = jsonable_encoder(users, by_alias=True)
...
async def create_user(
    *,
    ...
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(db_obj, by_alias=True)
...
async def read_user_by_id(
    ...
    stmt = (
        select(MasterUser)
        .where(MasterUser.user_id == user_id)
        .options(joinedload(MasterUser.role_rel))
    )
    result = await db.execute(stmt)
    user = result.scalars().first()
    if not user:
    ...
    # Check role through role_rel for 3NF compatibility
    if not current_user.role_rel or current_user.role_rel.role_name != "admin":
        raise HTTPException(
...
async def grant_user_access(
    *,
    ...
    if existing_access:
        ...
        # UPDATE 2.4.1 Pastikan response menggunakan camelCase
        return jsonable_encoder(UserBoardAccessResponse(
    ...
    access = UserBoardAccess(user_id=user_id, board_id=access_in.board_id)
    db.add(access)
    await db.commit()
...
async def revoke_user_access(
    *,
    ...
    """
    stmt = select(UserBoardAccess).where(
        UserBoardAccess.user_id == user_id, UserBoardAccess.board_id == board_id
    )
    result = await db.execute(stmt)
    ...
    await db.execute(
        delete(UserBoardAccess).where(UserBoardAccess.access_id == access.access_id)
    )
    await db.commit()
    ...
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder({"message": "Access revoked successfully"}, by_alias=True)
...
async def read_user_access(
    ...
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(access_list, by_alias=True)
