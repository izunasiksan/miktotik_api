...
async def list_jobs(
    ...
    return jsonable_encoder(jobs, by_alias=True)
...
async def trigger_auto_heal(
    ...
    try:
        result = await automation_service.trigger_auto_heal(
            db, heal_in.board_id, heal_in.reason, current_user.user_id
        )
        # UPDATE 2.4.1 Pastikan response menggunakan camelCase
...
@router.get("/ztp/queue/", response_model=List[ZTPResponse])
async def get_ztp_queue(
    ...
    """
    Mendapatkan daftar antrian perangkat ZTP yang pending.
    """
    results = await automation_service.get_ztp_queue(db, skip=skip, limit=limit)
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(results, by_alias=True)


@router.post("/ztp/approve/{ztp_id}/", response_model=dict)
async def approve_ztp_device(
    ztp_id: UUID,
    approve_in: ZTPApprove,
    db: AsyncSession = Depends(deps.get_db),
    current_user: MasterUser = Depends(deps.get_current_active_superuser),
) -> Any:
    """
    Menyetujui perangkat ZTP dan mendaftarkannya ke sistem.
    """
    try:
        board = await automation_service.approve_ztp_device(
            db,
            ztp_id=ztp_id,
            site_id=approve_in.site_id,
            model_id=approve_in.model_id,
            user_id=current_user.user_id,
        )
        # UPDATE 2.4.1 Pastikan response menggunakan camelCase
        return jsonable_encoder({"status": "success", "boardId": board.board_id}, by_alias=True)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.post("/ztp/reject/{ztp_id}/", response_model=dict)
async def reject_ztp_device(
    ztp_id: UUID,
    db: AsyncSession = Depends(deps.get_db),
    current_user: MasterUser = Depends(deps.get_current_active_superuser),
) -> Any:
    """
    Menolak perangkat ZTP.
    """
    try:
        await automation_service.reject_ztp_device(db, ztp_id=ztp_id)
        # UPDATE 2.4.1 Pastikan response menggunakan camelCase
        return jsonable_encoder({"status": "success"}, by_alias=True)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
