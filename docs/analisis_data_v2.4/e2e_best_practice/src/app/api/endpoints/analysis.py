...
from app.schemas.mikrotik import (
    AnalysisKpiSummaryResponse,
    ClientsAnalysisItem,
    HeavyAnalysisResponse,
    HotspotAnalysisItem,
    InterfaceAnalysisItem,
)
...
async def get_heavy_analysis(
    ...
    try:
        results = await analysis_service.get_heavy_analysis(
            db, board_id, days, start_time, end_time, granularity
        )
        ...
        if results:
            try:
                ...
                await redis_client.setex(
                    cache_key,
                    ttl,
                    json.dumps(jsonable_encoder(validated_results, by_alias=True)),
                )
                # UPDATE 2.4.1 Pastikan response menggunakan camelCase
                return jsonable_encoder(validated_results, by_alias=True)
            except Exception as e:
            ...
            return jsonable_encoder(
                HeavyAnalysisResponse.model_validate(empty_results), by_alias=True
            )
...
async def get_analysis_summary(
    ...
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(
        AnalysisKpiSummaryResponse.model_validate(results), by_alias=True
    )
...
async def get_interface_analysis(
    ...
    results = await analysis_service.get_interface_analysis(
        db, board_id, days, pivot_agg, start_time, end_time, granularity
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(
        [InterfaceAnalysisItem.model_validate(item) for item in results], by_alias=True
    )
...
async def get_pppoe_analysis(
    ...
    results = await analysis_service.get_pppoe_analysis(
        db, board_id, days, pivot_agg, start_time, end_time, granularity
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(
        [HotspotAnalysisItem.model_validate(item) for item in results], by_alias=True
    )
...
async def get_hotspot_analysis(
    ...
    results = await analysis_service.get_hotspot_analysis(
        db, board_id, days, pivot_agg, start_time, end_time, granularity
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(
...
async def get_interface_pivot(
    ...
    results = await analysis_service.get_interface_pivot(
        db, board_id, days, pivot_agg, start_time, end_time, granularity
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(results, by_alias=True)
...
async def get_pppoe_pivot(
    ...
    results = await analysis_service.get_pppoe_pivot(
        db, board_id, days, pivot_agg, start_time, end_time, granularity
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(results, by_alias=True)
...
async def get_hotspot_pivot(
    ...
    results = await analysis_service.get_hotspot_pivot(
        db, board_id, days, pivot_agg, start_time, end_time, granularity
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(results, by_alias=True)
...
async def get_clients_analysis(
    ...
    results = await analysis_service.get_clients_analysis(
        db, board_id, days, pivot_agg, start_time, end_time, granularity
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(
        [ClientsAnalysisItem.model_validate(item) for item in results], by_alias=True
    )
...
async def get_interface_forecast(
    ...
    """
    results = await analysis_service.get_interface_forecast(
        db, board_id, interface_name, days
    )
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(results, by_alias=True)
...
async def time_aggregate(
    ...
    metric: str = Query(
        "download_mbps", pattern="^(download_mbps|upload_mbps|cpu_load|free_memory)$"
    ),
    ...
    try:
        ...
        try:
            # UPDATE 2.4.1 Sinkronisasi Naming Convention (camelCase)
            data = jsonable_encoder(results, by_alias=True)
