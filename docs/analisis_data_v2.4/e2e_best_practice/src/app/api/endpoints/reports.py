...
@router.get("/speed-stats/{board_id}/", response_model=List[SpeedStatResponse])
async def get_speed_stats(
    ...
    """
    Get speed statistics (Mbps) for a specific board.
    Default limit: last 30 entries.
    """
    # Cache key generation
    s_time = start_time.isoformat() if start_time else "None"
    e_time = end_time.isoformat() if end_time else "None"
    cache_key = f"reports:speed:{board_id}:{limit}:{s_time}:{e_time}"
    ...
    try:
        ...
        if cached_data:
            return [
                SpeedStatResponse.model_validate(item)
                for item in json.loads(cached_data)
            ]
    except Exception as e:
        print(f"Redis error in get_speed_stats (get): {e}")

    query = select(BoardSpeedStat).where(BoardSpeedStat.board_id == board_id)
    ...
    query = query.order_by(desc(BoardSpeedStat.log_time))
    ...
    # Serialize and cache
    data = [
        SpeedStatResponse.model_validate(s).model_dump(by_alias=True)
        for s in stats
    ]
    try:
        await redis_client.setex(cache_key, 300, json.dumps(jsonable_encoder(data)))
    except Exception as e:
        print(f"Redis error in get_speed_stats (set): {e}")

    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(stats, by_alias=True)


@router.get("/resource-stats/{board_id}/", response_model=List[ResourceStatResponse])
async def get_resource_stats(
    ...
    """
    Get resource statistics (CPU, Memory, HDD, Uptime) for a specific board.
    Default limit: last 30 entries.
    """
    # Cache key generation
    s_time = start_time.isoformat() if start_time else "None"
    e_time = end_time.isoformat() if end_time else "None"
    cache_key = f"reports:resource:{board_id}:{limit}:{s_time}:{e_time}"
    ...
    try:
        ...
        if cached_data:
            return [
                ResourceStatResponse.model_validate(item)
                for item in json.loads(cached_data)
            ]
    except Exception as e:
        print(f"Redis error in get_resource_stats (get): {e}")
    ...
    query = query.order_by(desc(BoardResourceStat.log_time))
    ...
    # Serialize and cache
    data = [
        ResourceStatResponse.model_validate(s).model_dump(by_alias=True)
        for s in stats
    ]
    try:
        await redis_client.setex(cache_key, 300, json.dumps(jsonable_encoder(data)))
    except Exception as e:
        print(f"Redis error in get_resource_stats (set): {e}")
...
async def get_daily_reports(
    ...
    data = [
        DailySummaryResponse.model_validate(s).model_dump(by_alias=True)
        for s in summaries
    ]
    ...
    # UPDATE 2.4.1 Pastikan response menggunakan camelCase
    return jsonable_encoder(summaries, by_alias=True)
...
async def get_client_stats(
    ...
    try:
        ...
        if cached_data:
            return [
                ClientStatResponse.model_validate(item)
                for item in json.loads(cached_data)
            ]
    except Exception as e:
    ...
    # Serialize and cache
    data = [
        ClientStatResponse.model_validate(s).model_dump(by_alias=True) for s in stats
    ]
    try:
        await redis_client.setex(cache_key, 300, json.dumps(jsonable_encoder(data)))
    except Exception as e:
...
async def get_monthly_reports(
    ...
    try:
        ...
        if cached_data:
            return [
                MonthlySummaryResponse.model_validate(item)
                for item in json.loads(cached_data)
            ]
    except Exception as e:
    ...
    if start_time and end_time:
        query = query.where(
            and_(
                BoardMonthlySummary.log_month >= start_time.date(),
                BoardMonthlySummary.log_month <= end_time.date(),
            )
        )
    ...
    # Serialize and cache
    data = [
        MonthlySummaryResponse.model_validate(s).model_dump(by_alias=True)
        for s in summaries
    ]
    try:
        await redis_client.setex(
            cache_key, 86400, json.dumps(jsonable_encoder(data))
        )  # Monthly reports can be cached longer
    except Exception as e:
...
async def get_interface_reports(
    ...
    try:
        ...
        if cached_data:
            return [
                InterfaceUsageResponse.model_validate(item)
                for item in json.loads(cached_data)
            ]
    except Exception as e:
    ...
    # Serialize and cache
    data = [
        InterfaceUsageResponse.model_validate(s).model_dump(by_alias=True)
        for s in summaries
    ]
    try:
        await redis_client.setex(cache_key, 3600, json.dumps(jsonable_encoder(data)))
...
async def get_pppoe_reports(
    ...
    try:
        ...
        if cached_data:
            return [
                PppoeUsageResponse.model_validate(item)
                for item in json.loads(cached_data)
            ]
    except Exception as e:
    ...
    if start_time and end_time:
        query = query.where(
            and_(
                ...
                BoardPppoeUsage.log_date <= end_time.date(),
            )
        )
    ...
    # Serialize and cache
    data = [
        PppoeUsageResponse.model_validate(s).model_dump(by_alias=True)
        for s in summaries
    ]
    try:
        await redis_client.setex(cache_key, 3600, json.dumps(jsonable_encoder(data)))
...
async def get_hotspot_reports(
    ...
    try:
        ...
        if cached_data:
            return [
                HotspotUsageRawResponse.model_validate(item)
                for item in json.loads(cached_data)
            ]
    except Exception as e:
    ...
    # Serialize and cache
    data = [
        HotspotUsageRawResponse.model_validate(s).model_dump(by_alias=True)
        for s in summaries
    ]
    try:
        await redis_client.setex(cache_key, 3600, json.dumps(jsonable_encoder(data)))
...
async def export_reports(
    ...
    if type == "daily":
        ...
        query = select(BoardMonthlySummary).where(
            BoardMonthlySummary.board_id == board_id
        )
        if start_time and end_time:
            query = query.where(
                and_(
    ...
    if format == "csv":
        ...
        elements.append(
            Paragraph(
                f"{type.capitalize()} Report for Board {board_id}", styles["Title"]
            )
        )
        ...
        for row in data:
            ...
            for col in columns:
                val = getattr(row, col)
                if isinstance(val, (float, int)) and col not in [
                    "log_date",
                    "log_month",
                ]:
                    val = round(val, 2)
