import uuid
from sqlalchemy import Column, String, Integer, Boolean, DateTime, ForeignKey, func, Index, Date, Numeric, Text, Interval, BigInteger, Computed, UniqueConstraint
from sqlalchemy.dialects.postgresql import UUID, MACADDR, INET, ARRAY
...
class MikrotikBoard(Base):
    ...
    board_model = Column(String(50))
    site_group = Column(String(50), default='Umum')
    ...
    # Relationships
    credentials = relationship("BoardCredential", back_populates="board", uselist=False, cascade="all, delete-orphan")
    vpn_profiles = relationship("VPNProfile", back_populates="board", cascade="all, delete-orphan")
    client_stats = relationship("BoardClientStat", back_populates="board", cascade="all, delete-orphan")
    resource_stats = relationship("BoardResourceStat", back_populates="board", cascade="all, delete-orphan")
    speed_stats = relationship("BoardSpeedStat", back_populates="board", cascade="all, delete-orphan")
    daily_summaries = relationship("BoardDailySummary", back_populates="board", cascade="all, delete-orphan")
    interface_daily_summaries = relationship("BoardInterfaceDailySummary", back_populates="board", cascade="all, delete-orphan")
    monthly_summaries = relationship("BoardMonthlySummary", back_populates="board", cascade="all, delete-orphan")
    events = relationship("BoardEvent", back_populates="board", cascade="all, delete-orphan")
    user_access = relationship("UserBoardAccess", back_populates="board", cascade="all, delete-orphan")
    
    # New Relationships
    backups = relationship("BoardBackup", back_populates="board", cascade="all, delete-orphan")
    interface_configs = relationship("BoardInterfaceConfig", back_populates="board", cascade="all, delete-orphan")
    interface_usages = relationship("BoardInterfaceUsage", back_populates="board", cascade="all, delete-orphan")
    pppoe_usages = relationship("BoardPppoeUsage", back_populates="board", cascade="all, delete-orphan")
    hotspot_usages = relationship("HotspotUsageRaw", back_populates="board", cascade="all, delete-orphan")
    # Note: TelegramRecipients links to board, but usually accessed via board_id query. 
    # We can add relationship if needed.
    telegram_recipients = relationship("TelegramRecipient", back_populates="board", cascade="all, delete-orphan")

    __table_args__ = (
        Index('idx_board_logic', 'is_online', 'is_monitor', 'is_public_review'),
        Index('idx_board_site', 'site_group'),
    )
...
class BoardCredential(Base):
    ...
    password_mikrotik_encrypted = Column(Text, nullable=False) # Changed to Text based on schema
...
class BoardClientStat(Base):
    ...
    stat_id = Column(BigInteger, primary_key=True, autoincrement=True) # BigSerial
    board_id = Column(UUID(as_uuid=True), ForeignKey("mikrotik_boards.board_id", ondelete="CASCADE"))
    total_hotspot = Column(Integer, default=0)
    total_pppoe = Column(Integer, default=0)
    
    # Generated column support in SQLAlchemy
    total_active = Column(Integer, Computed('total_hotspot + total_pppoe', persisted=True))
...
class BoardResourceStat(Base):
    ...
    resource_id = Column(BigInteger, primary_key=True, autoincrement=True) # BigSerial
    board_id = Column(UUID(as_uuid=True), ForeignKey("mikrotik_boards.board_id", ondelete="CASCADE"))
...
class BoardSpeedStat(Base):
    ...
    speed_id = Column(BigInteger, primary_key=True, autoincrement=True) # BigSerial
    board_id = Column(UUID(as_uuid=True), ForeignKey("mikrotik_boards.board_id", ondelete="CASCADE"))
    interface_name = Column(String(100), nullable=False)
    download_mbps = Column(Numeric(10, 2), default=0) # Numeric
    upload_mbps = Column(Numeric(10, 2), default=0) # Numeric
    accuracy_pct = Column(Numeric(5, 2), server_default='100.00')
    log_time = Column(DateTime(timezone=True), server_default=func.now())
    ...
    __table_args__ = (
        Index('idx_speed_interface_time', 'board_id', 'interface_name', log_time.desc()),
...
class BoardDailySummary(Base):
    ...
    # Traffic Stats
    avg_download = Column(Numeric(10, 2), default=0)
    max_download = Column(Numeric(10, 2), default=0)
    total_download_bytes = Column(BigInteger, default=0)
    
    avg_upload = Column(Numeric(10, 2), default=0)
    max_upload = Column(Numeric(10, 2), default=0)
...
class BoardInterfaceDailySummary(Base):
    ...
    avg_download_mbps = Column(Numeric(10, 2), default=0)
    max_download_mbps = Column(Numeric(10, 2), default=0)
    p95_download_mbps = Column(Numeric(10, 2), default=0)
    
    avg_upload_mbps = Column(Numeric(10, 2), default=0)
    max_upload_mbps = Column(Numeric(10, 2), default=0)
    ...
    __table_args__ = (
        Index('idx_interface_summary_lookup', 'board_id', 'interface_name', log_date.desc()),
        UniqueConstraint('board_id', 'interface_name', 'log_date', name='unique_interface_daily'),
    )
...
class BoardMonthlySummary(Base):
    ...
    # Traffic Stats
    avg_download = Column(BigInteger, default=0)
    max_download = Column(BigInteger, default=0)
    total_download_bytes = Column(BigInteger, default=0)
    
    avg_upload = Column(BigInteger, default=0)
    max_upload = Column(BigInteger, default=0)
    total_upload_bytes = Column(BigInteger, default=0)
...
class BoardEvent(Base):
    ...
    event_id = Column(BigInteger, primary_key=True, autoincrement=True) # BigSerial
    board_id = Column(UUID(as_uuid=True), ForeignKey("mikrotik_boards.board_id", ondelete="CASCADE"))
    event_category = Column(String(20))
    event_level = Column(String(10))
    event_name = Column(Text, nullable=False)
    event_detail = Column(Text)
    performed_by = Column(UUID(as_uuid=True), ForeignKey("master_users.user_id", ondelete="SET NULL"))
...
class BoardBackup(Base):
    ...
    __table_args__ = (
        Index('idx_backup_board_history', 'board_id', log_date.desc()),
    )
...
class TelegramBot(Base):
    ...
    recipients = relationship("TelegramRecipient", back_populates="bot", cascade="all, delete-orphan")
...
class TelegramRecipient(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("master_users.user_id", ondelete="CASCADE"))
...
class BoardInterfaceUsage(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('board_id', 'interface_name', 'log_date', name='unique_daily_usage'),
    )
...
class BoardPppoeUsage(Base):
    ...
    __table_args__ = (
        Index('idx_pppoe_usage_lookup_v2', 'board_id', log_date.desc(), download_bytes.desc()),
        UniqueConstraint('board_id', 'pppoe_username', 'log_date', name='unique_pppoe_daily'),
    )
...
class HotspotUsageRaw(Base):
    ...
    __table_args__ = (
        Index('idx_hotspot_usage_lookup', 'board_id', log_date.desc(), daily_download.desc()),
        UniqueConstraint('username', 'board_id', 'log_date', name='unique_user_daily_raw'),
    )
