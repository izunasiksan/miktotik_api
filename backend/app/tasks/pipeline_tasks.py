...
@celery_app.task(bind=True, name="app.tasks.pipeline_tasks.execute_full_pipeline_v21_task")
def execute_full_pipeline_v21_task(self, board_id: str, start_time: str, end_time: str, granularity: str = "hour"):
    """
    Task Celery untuk menjalankan Full Pipeline V2.1 (Stage 1-7) secara asinkron.
    """
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(_run_pipeline(board_id, start_time, end_time, granularity, task_obj=self))

async def _run_pipeline(board_id: str, start_time: str, end_time: str, granularity: str, task_obj=None):
    async with SessionLocal() as db:
        try:
            ...
            update_progress(10, "Stage 0: Normalizing Data")
