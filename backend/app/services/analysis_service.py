...
async def cleanup_old_temp_tables(db: AsyncSession, max_age_minutes: int = 60):
    ...
    """
    try:
        # Cari tabel temporary di PostgreSQL
        # Note: Ini spesifik untuk PostgreSQL
        query = text("""
            SELECT tablename 
            ...
            if len(parts) >= 4:
                try:
                    ...
                    if (datetime.now().timestamp() - ts) > (max_age_minutes * 60):
                        logger.info(f"Cleaning up orphaned temp table: {table}")
...
async def create_scoped_dataset(
    ...
    if normalized_source and ("traffic" in normalized_source or "resource" in normalized_source):
        ...
        for t in traffic_data:
            ...
            if ts:
                combined_map[ts] = {
                    ...
                    "rx": t.get("rx", 0),
                    "tx": t.get("tx", 0),
                    "acc_traffic": t.get("accuracy_pct", 0),
                    "cpu": 0,
                    "mem": 0,
                    "acc_resource": 0,
                    "is_gap": t.get("isGap", False)
                }
            ...
            if ts:
                if ts in combined_map:
                    combined_map[ts].update({
                        "cpu": r.get("cpu", 0),
                        "mem": r.get("mem", 0),
                        "acc_resource": r.get("accuracy_pct", 0),
                        "is_gap": combined_map[ts]["is_gap"] or r.get("isGap", False)
                    })
                else:
                    combined_map[ts] = {
                        "period": ts,
                        ...
                        "mem": r.get("mem", 0),
                        "acc_resource": r.get("accuracy_pct", 0),
                        "is_gap": r.get("isGap", False)
                    }
        
        # Insert combined data into temp table
        if combined_map:
            insert_query = text(f"""
                INSERT INTO {temp_table_name} (period, rx, tx, acc_traffic, cpu, mem, acc_resource, is_gap)
                VALUES (:period, :rx, :tx, :acc_traffic, :cpu, :mem, :acc_resource, :is_gap)
            """)
            ...
            for ts, data in combined_map.items():
                values.append({
                    "period": datetime.fromisoformat(ts.replace("Z", "+00:00")),
                    "rx": float(data["rx"]),
                    "tx": float(data["tx"]),
                    "acc_traffic": float(data["acc_traffic"]),
                    "cpu": float(data["cpu"]),
                    "mem": float(data["mem"]),
                    "acc_resource": float(data["acc_resource"]),
            ...
            logger.warning("[STAGE 1] No data found in Normalized Source")

    else:
        # Fallback to old logic (Raw Stats) - V2.3 Legacy
        logger.warning("[STAGE 1] V2.4: Normalized Source missing! Falling back to Raw Stats (Legacy)")
        
        use_summary = (end_time - start_time).days > 365
        
        if use_summary:
            logger.info("[STAGE 1] Range > 365 days detected. Using BoardDailySummary for materialization.")
            query = text(f"""
    ...
    PIPELINE_STAGE_LATENCY.labels(stage_name="stage1_lock").observe(time.time() - start_ts)
...
async def get_trend_analysis(
    ...
    total_pts = summary[8] or 1
    gap_pts = summary[9] or 0
    # Dynamic Accuracy Calculation (V2.4)
    # accuracy = (1 - (gaps / total)) * 100
    dynamic_acc = (1.0 - (float(gap_pts) / float(total_pts))) * 100.0 if total_pts > 0 else 0.0
    ...
    PIPELINE_STAGE_LATENCY.labels(stage_name="stage2_trend").observe(time.time() - start_ts)
    
    # Calculate average accuracy from temp table (Legacy acc_traffic/acc_resource)
    query_acc_legacy = text(f"SELECT avg(acc_traffic) as acc_t, avg(acc_resource) as acc_r FROM {temp_table}")
    res_acc = await db.execute(query_acc_legacy)
    acc_row = res_acc.fetchone()
    avg_acc_legacy = (float(acc_row[0] or 100.0) + float(acc_row[1] or 100.0)) / 2 if acc_row else 100.0
    ...
    return {
        "summary": {
            ...
            "data_quality": {
                ...
                "gap_percentage": round((gap_pts / total_pts) * 100, 2) if total_pts > 0 else 0
            }
        },
        "series": series,
        "metadata": {
...
async def get_advanced_analytics(
    ...
    PIPELINE_STAGE_LATENCY.labels(stage_name="stage3_5_analytics").observe(time.time() - start_ts)
    if len(anomalies) > 0:
...
async def calculate_health_score(
    ...
    res = {
        ...
        "metadata": {
            "is_low_confidence": (trend_data.get("metadata", {}).get("accuracy_pct", 100) < 100),
            "accuracy_pct": trend_data.get("metadata", {}).get("accuracy_pct", 100)
        }
    }
    
    PIPELINE_STAGE_LATENCY.labels(stage_name="stage6_scoring").observe(time.time() - start_ts)
    return res
...
async def generate_insights(
    ...
    metadata = {
        "source_id": analytics_data.get("metadata", {}).get("source_id", "SystemEngine"),
        "accuracy_pct": health_score.get("metadata", {}).get("accuracy_pct", 100),
        "is_low_confidence": health_score.get("metadata", {}).get("is_low_confidence", False),
    ...
    PIPELINE_STAGE_LATENCY.labels(stage_name="stage7_insight").observe(time.time() - start_ts)
    return insights
...
async def get_heavy_analysis(
    ...
    percentile_query = text("""
        SELECT 
            ...
            (SELECT log_time FROM board_speed_stats WHERE board_id = :board_id AND download_mbps = (SELECT max(download_mbps) FROM board_speed_stats WHERE board_id = :board_id AND log_time >= :start_time AND log_time < :end_time) AND log_time >= :start_time AND log_time < :end_time LIMIT 1) as max_dl_date,
            (SELECT log_time FROM board_speed_stats WHERE board_id = :board_id AND upload_mbps = (SELECT max(upload_mbps) FROM board_speed_stats WHERE board_id = :board_id AND log_time >= :start_time AND log_time < :end_time) AND log_time >= :start_time AND log_time < :end_time LIMIT 1) as max_ul_date
        FROM board_speed_stats
    ...
    try:
        ...
        params = {"board_id": board_id, "start_time": start_time, "end_time": end_time, "granularity": granularity}
