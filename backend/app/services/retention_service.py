...
from app.models.mikrotik import (
    BoardResourceStat, BoardEvent, BoardBackup,
    BoardClientStat, BoardSpeedStat, HotspotUsageRaw,
    BoardPppoeUsage, BoardInterfaceUsage
)
...
async def prune_raw_data(days_retention: int = 30):
    ...
    async with SessionLocal() as db:
        ...
        for model, col, name in tables_to_prune:
            try:
                stmt = delete(model).where(col < (cutoff_date if "log_time" in col.key else cutoff_date_only))
                result = await db.execute(stmt)
                await db.commit()
                deleted_count = int(getattr(result, "rowcount", 0))
