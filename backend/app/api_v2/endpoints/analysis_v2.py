...
async def execute_pipeline_v21(
    ...
    # P4: Analysis Caching (Medium Term Recommendation)
    cache_key = f"analysis_v21_{board_id}_{start_time.isoformat()}_{end_time.isoformat()}_{granularity}"
    cached_res = await redis_client.get(cache_key)
    if cached_res:
        logger.info(f"Returning cached analysis for board {board_id}")
        try:
            return AnalysisResponse.model_validate(json.loads(cached_res))
        except Exception as e:
            logger.warning(f"Cache validation error: {e}")
            # If cache is invalid, proceed to recalculate
    ...
    try:
        ...
        try:
            ...
            try:
                # Use jsonable_encoder to handle UUID/Datetime
                cache_payload = validated_payload.model_dump(by_alias=False)
                cache_payload["metadata"]["cached"] = True
                await redis_client.setex(cache_key, 3600, json.dumps(jsonable_encoder(cache_payload)))
...
async def get_task_status(
    ...
    if state == "PROGRESS":
        ...
        if isinstance(info, dict):
            ...
            response["current_stage"] = info.get("stage", "")
    elif state == "SUCCESS":
        response["result"] = res.result
        response["progress"] = 100.0
        response["current_stage"] = "Completed"
    elif state == "FAILURE":
...
@router.get("/{board_id}/aggregate-all/", response_model=Union[List[AggregateAllItem], AggregateAllResponse])
