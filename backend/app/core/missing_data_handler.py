...
class MissingDataHandler:
    def __init__(self, threshold_caution: float = 0.05, threshold_risky: float = 0.15, threshold_critical: float = 0.30):
    ...
    def detect_missing_data(self, data: List[Optional[float]]) -> Dict[str, Any]:
        ...
        missing_count = sum(1 for x in data if x is None or (isinstance(x, float) and np.isnan(x)))
    ...
    def select_strategy(self, data_type: str, missing_percentage: float, is_timeseries: bool = True) -> ImputationStrategy:
        """
        ...
        if missing_percentage <= self.threshold_caution:
            # 1% - 5%: FFill or Linear
            return ImputationStrategy.FORWARD_FILL if is_timeseries else ImputationStrategy.MEAN
        ...
        return ImputationStrategy.LINEAR_INTERPOLATION if is_timeseries else ImputationStrategy.MEDIAN

    def handle_missing_data(self, data: List[Optional[float]], strategy: ImputationStrategy) -> List[float]:
        """
    ...
    def validate_imputation(self, original_valid_data: List[float], imputed_data: List[float]) -> Dict[str, Any]:
        """
