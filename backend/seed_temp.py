...
# Configuration
# Changed to connect to 'db' host for Docker network
DB_URL = "postgresql://postgres:root@db:5432/db_master_mikrotik"
NUM_BOARDS = 5
NUM_USERS = 7
        ...
        while current_time <= end_time:
            ...
            for bid in board_ids:
                ...
                for hour in [0, 6, 12, 18]:
                    ...
                    # V2.1: Default accuracy_pct = 100.0 for seeded data
                    await conn.execute(
                        "INSERT INTO board_client_stats (board_id, total_hotspot, total_pppoe, log_time, accuracy_pct) VALUES ($1, $2, $3, $4, $5)",
                        bid, hotspot, pppoe, sample_time, 100.0
                    )
                    ...
                    await conn.execute(
                        "INSERT INTO board_resource_stats (board_id, cpu_load, free_memory, free_hdd, log_time, accuracy_pct) VALUES ($1, $2, $3, $4, $5, $6)",
                        bid, cpu, mem, 1024*1024*1024, sample_time, 100.0 # 1GB HDD
                    )
                    ...
                    await conn.execute(
                        "INSERT INTO board_speed_stats (board_id, interface_name, download_mbps, upload_mbps, log_time, accuracy_pct) VALUES ($1, $2, $3, $4, $5, $6)",
                        bid, "ether1", dl, ul, sample_time, 100.0
                    )
                ...
                if random.random() < 0.1: # 10% chance per day
                    await conn.execute(
                        "INSERT INTO board_events (board_id, event_category, event_level, event_name, event_detail, log_time) VALUES ($1, $2, $3, $4, $5, $6)",
                        bid, "system", "warning", "High CPU Load", "CPU load exceeded 90% for 5 minutes", 
        ...
        for bid in board_ids:
            for i in range(20): # 20 users
                ...
                for d in range(7): # 7 days
                    ...
                    await conn.execute(
                        "INSERT INTO hotspot_usage_raw (board_id, username, daily_download, daily_upload, daily_uptime, log_date) VALUES ($1, $2, $3, $4, $5, $6)",
                        bid, username, dl_bytes, ul_bytes, random.randint(3600, 28800), usage_date
                    )
